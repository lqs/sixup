...
| `-dhcp6c-pd-grace` | `10s` | 启动后等待委派前缀的最长时间。PD 一旦成功或被拒绝，等待就结束，通常只需几秒。在此期间，RA 前缀不分给内网，因此以后不必再撤回。RA 设置了 M 或 O 时，WAN 也暂不在该前缀中配置地址，因为内网是否共享这个 /64 决定了地址的前缀长度。 |
...
| `-ra-pref64` | `auto` | 在 RA 中通告的 NAT64 前缀（RFC 8781）。`auto` 在本机 NAT64 转换时通告它的前缀 `-nat64-prefix`，否则转发上游 RA 中的值。`off` 不通告。写一个前缀则通告该前缀，用于别处的 NAT64；不能与 `-nat64` 同时使用。 |
...
| 选项 | 默认值 | 说明 |
|---|---|---|
| `-dhcp6s-mode` | `off` | `stateless` 不分配地址，只回答 DNS 等选项；`stateful` 还分配地址；两者都向下游路由器委派前缀（`-dhcp6s-pd-len`）；`off` 不运行服务器。RA 中的标志随之设置。 |
| `-dhcp6s-pool` | `1000-ffff` | 分配地址所用的接口标识范围，十六进制，指低 64 位。 |
| `-dhcp6s-static` | | 固定分配，可重复：`mac=<MAC>,addr=::100` 或 `duid=<十六进制>,addr=2001:db8::5`。 |
| `-dhcp6s-lease-preferred` | `45m` | 分配的地址和委派的前缀的首选生存期，不超过上游剩余的时长。默认值是 RFC 9096 建议的上限。 |
| `-dhcp6s-lease-valid` | `90m` | 分配的地址和委派的前缀的有效生存期，同样受上游限制。 |
| `-dhcp6s-pd-len` | `60` | 委派给下游路由器的最大前缀，以前缀长度表示。下游提示更长的前缀时按提示给；提示更短的前缀时给这个长度，空间不够时再加长。前缀从上游委派的前缀中划出，避开内网子网，两种服务器模式下都可用。`0` 关闭下游委派。 |

### NDP 代理

| 选项 | 默认值 | 说明 |
|---|---|---|
| `-ndproxy-mode` | `auto` | `auto` 在内网的 /64 同时是广播型 WAN 上的 on-link /64 时启用 `forward`，否则关闭；委派来的前缀和点对点 WAN 都不需要代理。`forward` 先到另一侧探测再应答，双向进行，因此 WAN 链路上的主机和内网主机之间也能互访。`prefix` 对 WAN 侧询问内网前缀内任意地址的请求一律应答，不做探测。`static` 只把 `-ndproxy-static` 的条目写进内核代理表。`off` 关闭代理。 |
...
| 选项 | 默认值 | 说明 |
|---|---|---|
| `-nat64` | `off` | `jool` 用 [Jool](https://jool.mx) 内核模块（4.1 或更新版本，用 `modprobe jool` 加载）配置 NAT64。Jool 运行在独立的网络命名空间里，经 veth `sixup-nat64` 相连，转换 `-nat64-prefix`，内网和路由器本身都能使用。只有模块加载时才创建该命名空间，并在 RA 中通告这个前缀（`-ra-pref64 off` 时不通告）；sixup 每分钟检查一次，加载、卸载或重新加载模块都不需要重启。转换出的 IPv4 按本机的 IPv4 路由出去：走隧道时经过上面的源地址转换，MAP-E 线路的端口只由一方分配；没有隧道时从已配置的其他 IPv4 出口出去。防火墙需要放行经过 `sixup-nat64` 的流量：既有转发的内网流量，也有发给路由器自身、从这个接口进来的回包。该命名空间随 sixup 一起消失。`off` 不配置。DNS64 不在其中，由你选择的解析器提供。 |
| `-nat64-prefix` | `64:ff9b::/96` | NAT64 转换的前缀（RFC 6052），长度必须是 32、40、48、56、64 或 96。要通过 NAT64 访问私有 IPv4 地址，需要用网络专用前缀，例如 ULA `fd00:64::/96`；从委派前缀中划出的前缀会随之变化，不宜使用。 |
| `-jool-ipv4` | `192.168.255.254/31` | 本命名空间与 Jool 所在命名空间之间的 IPv4 /31。较小的地址是本侧的网关，较大的地址是 Jool 的 pool4。与本机已有的地址或路由重叠时，sixup 拒绝启动。 |
