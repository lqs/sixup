...
| `-ra-pref64` | | 通告的 NAT64 前缀（RFC 8781），如 `64:ff9b::/96`。留空则转发上游 RA 中的值。长度必须是 32、40、48、56、64 或 96。配合 `-nat64 jool` 时，它改为 Jool 转换的前缀，留空即 `64:ff9b::/96`，并且只在 Jool 运行时通告。要通过 NAT64 访问私有 IPv4 地址，需要用网络专用前缀，例如 ULA `fd00:64::/96`（RFC 6052）；从委派前缀中划出的前缀会随之变化，不宜使用。 |
...
| 选项 | 默认值 | 说明 |
|---|---|---|
| `-nat64` | `off` | `jool` 用 [Jool](https://jool.mx) 内核模块（4.1 或更新版本，用 `modprobe jool` 加载）配置 NAT64。Jool 运行在独立的网络命名空间里，经 veth `sixup-nat64` 相连，转换 `64:ff9b::/96` 或 `-ra-pref64` 指定的前缀，内网和路由器本身都能使用。只有模块加载时才创建该命名空间并在 RA 中通告这个前缀；sixup 每分钟检查一次，加载、卸载或重新加载模块都不需要重启。转换出的 IPv4 按本机的 IPv4 路由出去：走隧道时经过上面的源地址转换，MAP-E 线路的端口只由一方分配；没有隧道时从已配置的其他 IPv4 出口出去。防火墙需要放行经过 `sixup-nat64` 的流量：既有转发的内网流量，也有发给路由器自身、从这个接口进来的回包。该命名空间随 sixup 一起消失。`off` 不配置。DNS64 不在其中，由你选择的解析器提供。 |
